...
async function runLiveBacktest() {
  ...
  for (const symbol of topCoins) {
    ...
    for (const strategy of coinStrategies) {
      ...
      if (results.totalTrades > 0) {
        console.log(
          `✅ ${strategy.name}: ${results.totalTrades} trades | Win Rate: ${results.winRate.toFixed(1)}% | PNL: $${results.totalPnL.toFixed(2)}`,
        );
      } else {
  ...
  console.log(
    `${'Strategy'.padEnd(30)} | ${'Trades'.padEnd(8)} | ${'Win%'.padEnd(8)} | ${'Net PNL'.padEnd(10)}`,
  );
  console.log(
    `${'-'.repeat(30)}-|-${'-'.repeat(8)}-|-${'-'.repeat(8)}-|-${'-'.repeat(10)}`,
  );
  ...
  Object.entries(summary).forEach(([name, stats]) => {
    ...
    console.log(
      `${name.padEnd(30)} | ${stats.trades.toString().padEnd(8)} | ${winRate.toFixed(1).padEnd(7)}% | $${stats.pnl.toFixed(2).padEnd(9)}`,
    );
  ...
  console.log(`${'-'.repeat(60)}`);
  console.log(
    `${'TOTAL'.padEnd(30)} | ${totalTrades.toString().padEnd(8)} | ${'N/A'.padEnd(8)} | $${totalPnL.toFixed(2)}`,
  );
  console.log(`${'='.repeat(60)}`);
