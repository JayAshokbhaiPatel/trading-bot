    ...
    const timeframeSets = [
      {
        name: '4-Hour',
        candles: tf4h,
        hTrend: 'SIDEWAYS' as any,
        mTrend: 'SIDEWAYS' as any,
      }, // Higher TF self-validates
      {
        name: '1-Hour',
        candles: tf1h,
        hTrend: trend4h,
    ...
    for (const tfSet of timeframeSets) {
      for (const strategy of strategies) {
        ...
        if (signal && signal.action !== 'WAIT') {
          ...
          const trendMatch = this.checkTrendConfluence(
            signal.action,
            tfSet.mTrend,
            tfSet.hTrend,
          );
          ...
          if (trendMatch) {
            ...
            if (livePrice > 0) {
              ...
              if (signal.action === 'BUY' && livePrice <= signal.stopLoss)
                continue;
              if (signal.action === 'SELL' && livePrice >= signal.stopLoss)
                continue;
            ...
            if (
              !bestResult ||
              result.confluenceScore > bestResult.confluenceScore
            ) {
              bestResult = result;
            }
          }
