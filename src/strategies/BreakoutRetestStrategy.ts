  ...
  public analyze(candles: Candle[]): Signal {
    ...
    if (this.breakoutOccurred) {
      ...
      if (
        ...
        Math.abs(currentPrice - this.resistanceLevel) / this.resistanceLevel <
          0.01
      ) {
        // Loosened from 0.002
      ...
      if (
        this.supportLevel &&
        Math.abs(currentPrice - this.supportLevel) / this.supportLevel < 0.01
      ) {
        // Loosened from 0.002
        this.breakoutOccurred = false; // Reset
        return {
      ...
      if (this.resistanceLevel && currentPrice > this.resistanceLevel * 1.05)
        this.breakoutOccurred = false;
      if (this.supportLevel && currentPrice < this.supportLevel * 0.95)
        this.breakoutOccurred = false;
    }
