...
export class InsideBarStrategy extends BaseStrategy {
  ...
  public analyze(candles: Candle[]): Signal {
    ...
    const isInside =
      insideBar.high < motherBar.high && insideBar.low > motherBar.low;
    ...
    if (candles.length >= 3) {
      ...
      if (
        potentialInside.high < potentialMother.high &&
        potentialInside.low > potentialMother.low
      ) {
        // Bullish Breakout
        if (current.close > potentialMother.high) {
          return {
            ...
            takeProfit:
              current.close + (current.close - potentialMother.low) * 1.5,
            confidence: 0.7,
        ...
        if (current.close < potentialMother.low) {
          return {
            ...
            takeProfit:
              current.close - (potentialMother.high - current.close) * 1.5,
            confidence: 0.7,
            pattern: 'Inside Bar Breakout',
